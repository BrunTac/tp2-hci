<template>
    <div id="app">
        <button :disabled="securityStore.isLoggedIn" @click="login()">Login</button>
        <button :disabled="!securityStore.isLoggedIn" @click="getCurrentUser()">Current User</button>
        <button :disabled="!securityStore.isLoggedIn" @click="logout()">Logout</button>
        <label for="email">Email:</label><input v-model="email" name="email" />
        <label for="password">Password:</label><input name="password" v-model="password" type="password" />
        <br>
        <textarea id="result" v-model="result"></textarea><br />
        <span>Enter o pase card JSON in text area to create a new card</span><br />
        <button :disabled="!canCreate" @click="createCard()">Create Card</button>
        <button :disabled="!canGetAll" @click="getAllCards()">Get All Cards</button>
        <button :disabled="!canAbort" @click="abort()">Abort</button><br>
        <div v-if="securityStore.isLoggedIn" class="cards">
